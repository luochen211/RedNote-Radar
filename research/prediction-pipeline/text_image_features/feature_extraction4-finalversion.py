...
def safe_translate(text, src='zh-cn', dest='en', retries=3, delay=2):
    """带有重试机制的翻译函数"""
    for i in range(retries):
        try:
            ...
            print(f"翻译出错: {e}. 正在重试 {i+1}/{retries} 次...")
            time.sleep(delay)  # 延迟再试
    return text  # 如果所有重试失败，则返回原文本


# sample1 = "测试翻译工具"
# # 进行第一次翻译，目标是韩文
# translations =  translator.translate([sample1], 'ko')
# # 获得翻译结果
# print("中间翻译结果是",translations)

# async def safe_translate(text, src='zh-cn', dest='en', retries=5, delay=2):
#     """带有重试机制的异步翻译函数"""
#     for i in range(retries):
#         try:
#             # 使用await调用异步翻译方法
...
#         except Exception as e:
#             print(f"翻译出错: {e}. 正在重试 {i+1}/{retries} 次...")
#             await asyncio.sleep(delay)  # 使用异步sleep
#     return text  # 如果所有重试失败，则返回原文本
...
def process_data_file(input_file_path):
    ...
    for idx, item in enumerate(data):
        ...
        try:
            ...
            for tag in tags_list:
                tag = tag.strip()
                translated_tag = safe_translate(tag)
                text = clip.tokenize(translated_tag).to(device)
        ...
        # 5. VADER情感分析
        if title_text != 'nan':
            translated_text = safe_translate(title_text)
