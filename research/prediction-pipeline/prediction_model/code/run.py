...
class Run():
    ...
    def get_dataloader(self, data_type):
        ...
        if data_type=='XIAOHONGSHU':
            if self.train_scope =='all':
                ...
                # dev_maxvlue_dict = dataset_dev.max_value_dict
                collate_fn = XIAOHONGSHU_collate_fn
            # dataset_train = XIAOHONGSHUDataset(self.config, 'train')
            # train_maxvlue_dict = dataset_train.max_value_dict
            # dataset_test = XIAOHONGSHUDataset(self.config, 'test')
            # test_maxvlue_dict = dataset_test.max_value_dict
            # # dataset_dev = XIAOHONGSHUDataset(self.config, 'dev')
            # # dev_maxvlue_dict = dataset_dev.max_value_dict
            # collate_fn=XIAOHONGSHU_collate_fn
        ...
        # dataloaders =  dict(zip(['train', 'test', 'dev','train_maxvlue_dict','test_maxvlue_dict','dev_maxvlue_dict'],[train_dataloader, test_dataloader, dev_dataloader,train_maxvlue_dict,test_maxvlue_dict,dev_maxvlue_dict]))
        dataloaders = dict(zip(['train', 'test', 'train_maxvlue_dict', 'test_maxvlue_dict'],[train_dataloader, test_dataloader, train_maxvlue_dict,test_maxvlue_dict]))
    ...
    def main(self):
        ...
        trainer = Trainer(args = self.config, model = self.model, device = self.device, lr = self.lr, dataloaders = dataloaders, epoches = self.epoches, dropout = self.dropout, weight_decay = self.weight_decay, mode = self.mode, model_name = self.model_name, event_num = self.event_num,
            epoch_stop = self.epoch_stop, save_param_path = self.save_param_dir+self.data_type+"/"+self.model_name+"/", writer = SummaryWriter(self.path_tensorboard+"/"), train_scope = self.train_scope)
